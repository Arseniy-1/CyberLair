...
public class InvincibilityCollisionHandler : CollisionHandler
{
    ...
    protected override void HandleCollision(Collider2D collider)
    {
        ...
        if (collider.TryGetComponent(out IMoveable moveable))
        {
            if (collider.TryGetComponent(out Bullet bullet))
            {
                if (bullet.gameObject.layer == LayerMask.NameToLayer(_friendlyBulletLayerName))
                    return;
                
                Vector2 currentVelocity = moveable.Rigidbody2D.velocity;
                ...
                moveable.Rigidbody2D.velocity = newVelocity;

                if (newVelocity.sqrMagnitude > 0) 
                {
                    float angle = Mathf.Atan2(newVelocity.y, newVelocity.x) * Mathf.Rad2Deg;
                    moveable.Rigidbody2D.transform.rotation = Quaternion.Euler(new Vector3(0, 0, angle));
                }
            }
        }
    }
