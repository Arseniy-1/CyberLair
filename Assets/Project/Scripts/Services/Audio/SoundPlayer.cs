...
namespace Project.Scripts.Services.Audio
{
    ...
    {
        private readonly List<Spawners.Audio.Audio> _audioDatas = new ();
        ...
        {
            foreach (Spawners.Audio.Audio audio in _audioDatas.ToList())
            {
                Stop(audio.AudioID);
            }
        ...
        public void Play(AudioID audioID)
        {
            ...
            Spawners.Audio.Audio audio = _audioSpawner.Spawn();
        ...
        private void PlayOneShot(Spawners.Audio.Audio audio) => audio.PlayOneShot();
    
        private void PlayLoop(Spawners.Audio.Audio audio) => audio.PlayLoop();

        private void Stop(AudioID audioID)
        {
            Spawners.Audio.Audio currentAudio = _audioDatas.FirstOrDefault(audio => audio.AudioID == audioID);
        ...
        private void RemoveAudio(Spawners.Audio.Audio audio)
        {
            audio.OnDestroyed -= RemoveAudio;
